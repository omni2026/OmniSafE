...
def _legacy_chat_completion(model, messages, temperature):
    ...
    if chat_completion is None:
        raise RuntimeError(
            "Planner requires either an injected OpenAI v1 client "
...
class Planner(object):
    ...
    def __init__(self, arg, is_log_example=False, temperature=0, client=None):
        ...
        if self.arg.domain == 'blocksworld':
            ...
            self.max_examples = 3
            self.num_plan_example = min(arg.num_plan_example, self.max_examples)
            # household only ships the trans-style (PDDL) examples; no-trans not supported.
    ...
    def query(self, content, is_append=False, temperature=None):
        ...
        if self.client is not None:
            raw = self.client.chat.completions.create(
                model=self.model, messages=question, temperature=effective_temperature
            )
            response_content = raw.choices[0].message.content
            response = {"choices": [{"message": {"content": response_content}}]}
        else:
            response = _legacy_chat_completion(self.model, question, effective_temperature)
