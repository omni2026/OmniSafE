...
MODELS = ["gpt-3.5-turbo","gpt-4", "gpt-3.5-turbo-16k", "gpt-5.5", "deepseek-v4-flash"]
        ...
        for j in range(max_num_refine + 1):
            ...
            if 'Yes' in valid_result:
                ...
                if domain == 'blocksworld':
                    ...
                    planning_problem = error_description + " Please find a new plan by considering the preconditions of each action (robot location, what is held, container open/closed). "
...
if __name__=="__main__":
    ...
    # OpenAI v1 client (the OMNISAFE adapter uses this same path). Configure
    # via environment variables OPENAI_API_KEY and (optionally) OPENAI_BASE_URL,
    # so the upstream CLI works against both api.openai.com and any
    # OpenAI-compatible gateway (DeepSeek, Qwen, …).
    api_key = os.getenv("OPENAI_API_KEY", "")
    base_url = os.getenv("OPENAI_BASE_URL", "https://api.deepseek.com") or None
    try:
        from openai import OpenAI as _OpenAIv1
        llm_client = _OpenAIv1(api_key=api_key, base_url=base_url)
    except ImportError:
        # Legacy openai<1.0 fallback: set the module-level api_key so the
        # Translator/Planner/Validator fall back to openai.ChatCompletion.
    ...
    LLM_Translator = Translator(args, is_log_example=True, client=llm_client)

    # Initialize planner
    LLM_Planner = Planner(args, is_log_example=True, client=llm_client)

    # Initialize validator
    if args.method == "LLM_trans_self_feedback" or args.method == "LLM_no_trans_self_feedback":
        LLM_Validator = Validator(args, is_log_example=True, client=llm_client)
    ...
    with open(test_log_file_path, "w") as f:
        if args.domain == "blocksworld":
            ...
            f.write("Test log for "+ args.domain + " (driven by the OMNISAFE adapter; main.py CLI is a stub for this domain)." +"\n")
