...
def load_test_scenarios(args):
    ...
    if args.domain == 'blocksworld':
        ...
        # Household scenarios are driven by the OMNISAFE adapter, not by the
        # upstream CLI. Returning empty arrays here keeps `--domain household`
        # from crashing if a user happens to invoke main.py directly; the CLI
        # loop will simply have nothing to iterate over.
        initial_state = np.zeros((0, 0), dtype=int)
        goal_state = np.zeros((0, 0), dtype=int)
...
def extract_state_pddl(pddl_problem, domain):
    ...
    if domain == 'blocksworld':
        ...
        # Same shape as the cooking branch: (:init ...\n) and (:goal (and ...))\n.
        pddl_init_state = pddl_problem.split('(:init', 1)
        pddl_init_state = pddl_init_state[1]
        pddl_init_state = pddl_init_state.split('\n)', )
        pddl_init_state = pddl_init_state[0]
...
def extract_action_description(action_sequence, domain):
    ...
    if domain == 'blocksworld':
        ...
        actions = re.findall(r'\(.*?\)', action_sequence)
        # Drop atoms that are not action calls (e.g. type annotations or
        # accidental predicate echoes inside the response).
        household_actions = {'navigate', 'pick-up', 'place', 'open', 'close', 'cook', 'slice'}
        filtered_actions = []
        for atom in actions:
            head = atom.strip('()').split(' ', 1)[0]
            if head in household_actions:
                filtered_actions.append(atom)
        action_description = ""
        for atom in filtered_actions:
            action_description = action_description + atom + '\n'
